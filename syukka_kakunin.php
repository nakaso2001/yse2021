...
foreach($_POST["books"] as $book){
	...
	$totals=$books_count["stock"]-$_POST["stock"][$book_count];
...
if(isset($_POST["add"] )&& $_POST["add"]=="ok"){
	...
	foreach($_POST["books"] as $book){
		...
		$books_count=getByid($book,$pdo);
		//㉗ ㉖で取得した書籍の情報の「stock」と、㉔の変数を元にPOSTの「stock」から値を取り出して書籍情報の「stock」から値を引いた値を変数に保存する。
		$totals=$books_count["stock"]-$_POST["stock"][$books_number];
		//㉘「updateByid」関数を呼び出す。その際に引数に㉕の処理で取得した値と⑧のDBの接続情報と㉗で計算した値を渡す。
		updateByid($book,$pdo,$totals);
		//㉙ ㉔で宣言した変数をインクリメントで値を1増やす。
		$books_number++;
	}
		...
		<div id="center">
			<table>
				...
				<tbody>
					...
					foreach($_POST["books"] as $book){
						//㉞「getByid」関数を呼び出し、変数に戻り値を入れる。その際引数に㉜の処理で取得した値と⑧のDBの接続情報を渡す。
						$book_kazu=getByid($book,$pdo);
					?>
					<tr>
						<td><?php echo	/* ㉟ ㉞で取得した書籍情報からtitleを表示する。 */$book_kazu["title"];?></td>
						<td><?php echo	/* ㊱ ㉞で取得した書籍情報からstockを表示する。 */$book_kazu["stock"];?></td>
						<td><?php echo	/* ㊲ POSTの「stock」に設定されている値を㉜の変数を使用して呼び出す。 */$_POST["stock"][$books_counters];?></td>
					</tr>
					<input type="hidden" name="books[]" value="<?php echo /* ㊳ ㉝で取得した値を設定する */$book;?>">
					<input type="hidden" name="stock[]" value='<?php echo /* ㊴「POSTの「stock」に設定されている値を㉜の変数を使用して設定する。 */$_POST["stock"][$books_counters];?>'>
					<?php
						//㊵ ㉜で宣言した変数をインクリメントで値を1増やす。
						$books_counters++;
					}
